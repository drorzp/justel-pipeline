...
def process_files(input_dir, output_dir, start_delimiter, end_delimiter, skip_delimiter=False):
    ...
    # Get list of all txt and html files in the input directory
    files = [f for f in os.listdir(input_dir) if f.endswith(('.txt', '.html'))]
    ...
    for file in files:
        # Try UTF-8 first, then fall back to Latin-1 for older HTML files
        try:
...
###            pattern = re.compile(f'{re.escape(start_delimiter)}.*?{re.escape(end_delimiter)}', re.DOTALL) ### REPLACE PREVIOUS LINE FOR CASE-SENSITIVITY (REGARDING THE CONTENT OF THE DELIMITERS)
        ...
        content = content.replace("<!doctype html>", "[µµAAAµµ]<!doctype html>")
        content = content.replace("<h2", "[µµBBBµµ]<h2")
        content = content.replace("<A NAME=", "<BR><BR>**START**<A NAME=")
...
# Usage:
process_files('data/text_input', 'output/1', '<table', '</table>', skip_delimiter=True)
