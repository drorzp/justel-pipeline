...
###            pattern = re.compile(f'{re.escape(start_delimiter)}.*?{re.escape(end_delimiter)}', re.DOTALL) ### REPLACE PREVIOUS LINE FOR CASE-SENSITIVITY (REGARDING THE CONTENT OF THE DELIMITERS)
        ...
        # Check if this file contains Belgian footnote patterns
        has_footnotes = any('<sup><font color=red>' in match and '</font></sup>' in match for match in matches)

        if has_footnotes:
            log.write(f"File: {file} - PRESERVING BELGIAN FOOTNOTES (found {len(matches)} footnote patterns)\n\n")
            # Don't remove Belgian footnote content - preserve it
        else:
...
# Usage:
process_files('output/10', 'output/11', 'logs/log_clean3-2.txt', '<table', '</table>', skip_delimiter=True)
