...
###            pattern = re.compile(f'{re.escape(start_delimiter)}.*?{re.escape(end_delimiter)}', re.DOTALL) ### REPLACE PREVIOUS LINE FOR CASE-SENSITIVITY (REGARDING THE CONTENT OF THE DELIMITERS)
        ...
        content = content.replace("<BR><BR>**START**<A NAME=", "[µµBBBµµ]<BR><BR>**START**<A NAME=")
        content = content.replace("<table", "[µµBBBµµ]<table")
        content = content.replace("</div>", "[µµBBBµµ]</div>")
        content = content.replace(">------<BR>", ">[µµAAAµµ][µµAAAµµ]------<BR>")
        content = content.replace(">-------<BR>", ">[µµAAAµµ][µµAAAµµ]-------<BR>")
        content = content.replace(">--------<BR>", ">[µµAAAµµ][µµAAAµµ]--------<BR>")
        content = content.replace(">---------<BR>", ">[µµAAAµµ][µµAAAµµ]---------<BR>")
        content = content.replace(">----------<BR>", ">[µµAAAµµ][µµAAAµµ]----------<BR>")
        ...
        content = content.replace(";--------------<BR>", ";[µµAAAµµ][µµAAAµµ]--------------<BR>")
        content = content.replace(";---------------<BR>", ";[µµAAAµµ][µµAAAµµ]---------------<BR>")
        content = content.replace("]------<BR>", "][µµAAAµµ]------<BR>")
        content = content.replace("]-------<BR>", "][µµAAAµµ]-------<BR>")
        content = content.replace("]--------<BR>", "][µµAAAµµ]--------<BR>")
        content = content.replace("]---------<BR>", "][µµAAAµµ]---------<BR>")
        content = content.replace("]----------<BR>", "][µµAAAµµ]----------<BR>")
...
# Usage:
process_files('output/6', 'output/7', '<table', '</table>', skip_delimiter=True)
