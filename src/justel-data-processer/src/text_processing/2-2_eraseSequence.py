...
###            pattern = re.compile(f'{re.escape(start_delimiter)}.*?{re.escape(end_delimiter)}', re.DOTALL) ### REPLACE PREVIOUS LINE FOR CASE-SENSITIVITY (REGARDING THE CONTENT OF THE DELIMITERS)
        ...
        # MODIFIED: Check if content contains Belgian footnotes (dotted lines with numbered citations)
        has_footnotes = any('----------<BR>' in match and '<font color=red>' in match for match in matches)

        if has_footnotes:
            log.write(f"File: {file} - PRESERVING FOOTNOTES (found {len(matches)} footnote sections)\n\n")
            # Don't remove footnote content - preserve it
        else:
            # Original behavior for non-footnote content
...
# Usage:
# MODIFIED: Disabled footnote removal to preserve Belgian legal footnote system
# Original: process_files('output/7', 'output/8', 'logs/log_clean2-2.txt', '<table', '</table>', skip_delimiter=True)
# New approach: Skip the removal entirely by using a pattern that won't match footnotes
process_files('output/7', 'output/8', 'logs/log_clean2-2.txt', '<NEVER_MATCH_START>', '<NEVER_MATCH_END>', skip_delimiter=False)
